...
def calculate_predictive_parity(df, target_variable, prediction_variable, protected_variable_list, is_binary = True):
	"""
	Calculate the predictive parity metric for a given dataframe sensitive attribute
	...
	if not is_binary:
		...
		for protected_variable in protected_variable_list:
			...
			predictive_parity.loc[protected_variable] = calculate_precision(df_prot_subset, target_variable, prediction_variable)

		predictive_parity.loc["Gap","PredictiveParity"] = predictive_parity["PredictiveParity"].max() - predictive_parity["PredictiveParity"].min()
