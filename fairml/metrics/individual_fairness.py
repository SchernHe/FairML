...
def calculate_individual_fairness(model, df, columns ,switch_binaries, is_benchmark=False):

	if is_benchmark:
		df["Scores"] = model.predict_proba(df.iloc[:,columns])[:,1]
		df_manipulated = _manipulate(df,switch_binaries)
		df["Scores_Manipulated"] = model.predict_proba(df_manipulated.iloc[:,columns])[:,1]
	else:
...
def _manipulate(df,switch_binaries):
	...
	for feature in switch_binaries:
		df_manipulated[feature] = df.apply(lambda row: _switch_feature_value(row,feature),axis=1)
